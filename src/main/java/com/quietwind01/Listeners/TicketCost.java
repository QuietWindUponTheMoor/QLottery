...
public class TicketCost implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        // Debug
        //playerMessage(player, "TicketCost Args: " + args.length);

        // Check that the command has valid amount of arguments
        if (args.length < 2) {
            playerMessage(player, plugin.chatPrefix + "{yellow}Usage: /ql ticket cost");
            return true;
        }
        ...
        playerMessage(player, plugin.chatPrefix + "Tickets currently cost: {green}$" + formatNumber(plugin.getConfig().getDouble("ticket-cost")));
